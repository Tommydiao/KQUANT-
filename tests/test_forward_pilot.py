...
def _frozen_session(tmp_path: Path) -> tuple[Path, str]:
...
def test_forward_pilot_requires_frozen_strategy_and_preserves_candidates(tmp_path: Path) -> None:
    ...
    db, session_id = _frozen_session(tmp_path)
...
def test_paper_simulation_enforces_no_chasing_and_risk_limits(tmp_path: Path) -> None:
    db, session_id = _frozen_session(tmp_path)
    activate_forward_pilot(db, session_id)
    summary = record_forward_day(
        db_path=db, session_id=session_id, market_date="2026-07-24", preflight={"status": "pass"},
