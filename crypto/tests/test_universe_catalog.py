...
from kquant_crypto.universe_catalog import candidate_instrument, configured_cex_symbols, configured_instruments, cex_symbol_tiers, load_universe_catalog


def test_default_catalog_has_separate_cex_tiers_and_dex_chains(tmp_path):
    catalog = load_universe_catalog(tmp_path)

    assert catalog["version"] == "crypto_universe_v1.2.0"
    ...
    assert {"ZECUSDT", "ARBUSDT", "PUMPUSDT"} <= set(configured_cex_symbols(tmp_path))
    hype = candidate_instrument("HYPEUSDT")
    assert hype is not None
    assert hype.market_type == "perpetual"
    assert hype.risk_fraction_cap == 0.0025
    assert all(item.symbol != "HYPEUSDT" or item.market_type == "perpetual" for item in configured_instruments(tmp_path))
