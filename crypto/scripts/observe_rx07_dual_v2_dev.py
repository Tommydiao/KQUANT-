"""RX07 v2: rejected eligibility never promotes a bar or interrupts raw collection."""
import argparse
import asyncio
...
async def observe(out, seconds):
    ...
    manifest = dict(scope='DEV_ONLY_RECEIPT_V2', seconds=seconds, pid=os.getpid(),
        ...
        continuity='Single connection; transport/clock failure ends run; eligibility failures retained; no backfill',
            ...
            async with httpx.AsyncClient(timeout=8, trust_env=False) as client:
                ...
                async with connect(WS+'?streams='+streams, open_timeout=10, max_size=65536,
                    ...
                    if overdue:
                        ...
                        counts['overdue_boundaries'] = len(overdue)
    ...
    report = dict(status='FAILED' if failure else 'RAW_RECEIPT_SEGMENT_COMPLETE', failure=failure,
        ...
        quote_labels=0, receipt_only=True, strict_data_pass=False,
        eligibility_failures=counts['eligibility_failures'],
        qualification='Raw collection only; rejected/gapped bars never become usable history')
