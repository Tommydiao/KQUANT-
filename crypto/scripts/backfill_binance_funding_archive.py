...
from kquant_crypto.binance_archive import BinanceFundingArchiveBackfill
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import checksum-verified Binance USD-M funding archives.")
    ...
    results = BinanceFundingArchiveBackfill(
        ParquetMarketStore(settings.data_dir), workers=args.workers,
    ).run(args.symbols, start=args.start_month, end=args.end_month)
