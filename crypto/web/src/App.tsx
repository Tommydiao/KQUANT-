...
type Session = { authenticated: boolean; email?: string | null; configured: boolean; preview?: boolean };
...
type ExecutionStatus = {
  ...
  limits: {
    ...
    total_open_risk_fraction: number;
    max_leverage: number;
  };
  testnet_release_gate: { status: string; checks: Record<string, boolean>; observed: { calendar_days: number; closed_trades: number; fill_count: number; unsafe_orders: number } };
};
...
function Login({ onLogin }: { onLogin: (session: Session) => void }) {
  ...
  return <main className="auth-shell"><section className="auth-card">
    ...
    <h1>加密资产研究终端</h1><p className="muted">行情、策略验证与最终计划审核。交易执行默认锁定，且始终不接触钱包或提现权限。</p>
...
function Dashboard({ session, onLogout }: { session: Session; onLogout: () => void }) {
  ...
  const [error, setError] = useState("");
  useEffect(() => {
    Promise.all([getJson<Health>("/api/health"), getJson<SignalStatus>("/api/crypto/runtime/signal-status"), getJson<ValidationLatest>("/api/crypto/validation/latest"), getJson<{ items: Evaluation[] }>("/api/crypto/evaluations/latest"), getJson<NotificationStatus>("/api/notifications/status"), getJson<{ items: DexPair[] }>("/api/crypto/dex/pairs/latest?limit=12"), getJson<{ items: DexSecurity[] }>("/api/crypto/security/latest?limit=12"), getJson<SecurityCoverage>("/api/crypto/security/coverage"), getJson<EvidenceCoverage>("/api/crypto/evidence/coverage"), getJson<ModelBenchmarks>("/api/crypto/validation/model-benchmarks/latest"), getJson<{ items: RollDecision[] }>("/api/crypto/roll/current"), getJson<BayesianResult>("/api/crypto/research/bayesian/asset:ETH"), getJson<MonteCarloResult>("/api/crypto/research/monte-carlo/asset:ETH"), getJson<ExecutionStatus>("/api/crypto/execution/status")])
      .then(([nextHealth, nextSignalStatus, nextValidation, nextEvaluations, nextNotifications, nextDexPairs, nextDexSecurity, nextSecurityCoverage, nextEvidenceCoverage, nextModelBenchmarks, nextRolls, nextBayesian, nextMonteCarlo, nextExecution]) => { setHealth(nextHealth); setSignalStatus(nextSignalStatus); setValidation(nextValidation); setEvaluations(nextEvaluations.items); setNotificationStatus(nextNotifications); setDexPairs(nextDexPairs.items); setDexSecurity(nextDexSecurity.items); setSecurityCoverage(nextSecurityCoverage); setEvidenceCoverage(nextEvidenceCoverage); setModelBenchmarks(nextModelBenchmarks); setRollDecisions(nextRolls.items); setBayesian(nextBayesian); setMonteCarlo(nextMonteCarlo); setExecution(nextExecution); })
  ...
  return <main className="app-shell"><a className="mode-link gateway-link" href="http://127.0.0.1:8020/">KQUANT workspace gateway · Shadow {shadowSummary?.status ?? "loading"}</a>
    <header className="topbar"><div className="brand"><div className="brand-mark small">KQ</div><div><strong>KQUANT CRYPTO</strong><span>启动监测、验证与受控执行</span></div></div><div className="top-actions"><a className="mode-link" href="http://127.0.0.1:8001/">Stocks</a><span className="status-pill green">{session.preview ? "本机预览" : "研究可用"}</span><span className="status-pill amber">执行锁定</span>{!session.preview && <button className="quiet-button" onClick={logout}>退出</button>}</div></header>
    <div className="workspace"><aside className="sidebar"><p className="eyebrow">导航</p><button className="nav-item active">今日监测</button><button className="nav-item">市场状态</button><button className="nav-item">CEX 雷达</button><button className="nav-item">DEX / MEME</button><button className="nav-item">预警中心</button><button className="nav-item">数据可信度</button><div className="sidebar-footer"><span>{session.email}</span><small>所有动作均需经过 EVAL</small></div></aside>
      <section className="content"><div className="page-heading"><div><p className="eyebrow">今日监测</p><h1>{monitoringTitle}</h1><p className="muted">系统只使用闭合 K 线生成研究草案；任何候选都必须先经过确定性 EVAL，数据不足时保持观察状态。</p></div><span className="timestamp">策略版本：{signalStatus?.strategy_version ?? "loading"}</span></div>
        ...
        <section className="panel runtime-panel"><div className="panel-heading"><div><p className="eyebrow">BINANCE EXECUTION</p><h2>自动执行就绪度</h2></div><span className={`status-pill ${execution?.armed ? "green" : "amber"}`}>{execution?.armed ? "已 ARM" : "锁定"}</span></div><p className="muted">订单只能由通过历史验证、EVAL 与账户风险审查的策略生成；界面不提供任意手工下单入口。重启服务会自动解除 ARM。</p><div className="runtime-grid"><div><span>执行模式</span><strong>{execution?.mode ?? "loading"}</strong></div><div><span>自动交易</span><strong>{execution?.autotrade_enabled ? "已配置" : "关闭"}</strong></div><div><span>API 凭据</span><strong>{execution?.credentials_configured ? "已配置" : "未配置"}</strong></div><div><span>已验证策略</span><strong>{execution?.validated_strategies.length ? execution.validated_strategies.length : 0}</strong></div><div><span>单笔 / 总开放风险</span><strong>{execution ? `${execution.limits.risk_per_trade_fraction * 100}% / ${execution.limits.total_open_risk_fraction * 100}%` : "-"}</strong></div><div><span>每日亏损上限</span><strong>{execution ? `${execution.limits.daily_loss_fraction * 100}%` : "-"}</strong></div><div><span>Live 资金上限</span><strong>{execution ? `${execution.limits.capital_usdt} USDT` : "-"}</strong></div><div><span>永续最高杠杆</span><strong>{execution ? `${execution.limits.max_leverage}x` : "-"}</strong></div><div><span>Testnet 放行</span><strong>{execution?.testnet_release_gate.status ?? "未评估"}</strong><small>{execution ? `${execution.testnet_release_gate.observed.calendar_days.toFixed(1)} 天 / ${execution.testnet_release_gate.observed.closed_trades} 笔闭合交易` : ""}</small></div><div><span>Kill Switch</span><strong>{execution?.kill_switch_active ? "已触发" : "待命"}</strong></div></div>{execution?.blockers.length ? <div className="notice red"><strong>当前不可执行</strong><span>{execution.blockers.join("；")}</span></div> : <div className="notice green-notice">执行配置检查通过，仍需完成对应策略的 EVAL 与实时账户风险审查。</div>}</section>
